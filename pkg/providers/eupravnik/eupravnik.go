// Package eupravnik downloads the monthly eUpravnik building-maintenance
// invoice. eUpravnik has no API: the invoice arrives as a PDF attached to an
// email, so this provider reads the mailbox over IMAP (via the mailbox package)
// and saves the newest attachment like every other provider.
package eupravnik
...
// already contains only eUpravnik's mail, so no sender filter is needed. Set it
// to eUpravnik's address if you search a shared folder like INBOX instead.
var senderFilter = ""
...
// Service downloads the eUpravnik invoice from a mailbox.
type Service struct {
...
func (s *Service) DownloadReceipt() error {
	...
	s.logger.Info().Int("messages", len(messages)).Msg("Searched mailbox for eUpravnik invoice")
	...
	// receipt is filed under the month it bills for (e.g. Maj 2026 -> 05-2026)
	// rather than whenever the email happened to arrive.
	in, parseErr := parseInvoice(attachment.Data)
	if parseErr != nil {
		s.logger.Err(parseErr).Msg("Could not parse eUpravnik PDF; filing under email date, price/status left unset")
	}
...
//
// eUpravnik's "DUG" is the outstanding balance *before* this invoice, so a fresh
// invoice never proves itself paid — it proves the previous month is. The status
// model therefore is:
//
...
func (s *Service) recordInvoice(ctx context.Context, in invoice, period string) {
	...
	// A zero outstanding balance settles the previous month. Requires the parsed
