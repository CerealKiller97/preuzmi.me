...
- 🧾 **Plati sve** — a monthly batch-payment screen at `/pay`. Every unpaid receipt on one list, each with its IPS QR, a sticky running total, and one-tap **Označi kao plaćeno**. Defaults to the newest period that still has unpaid bills; switch periods (or view all) from the dropdown. Sit down once a month, scan down the list, done.
